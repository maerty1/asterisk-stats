/**
 * Адаптер для Bookshelf.js (основан на Knex)
 */
...
class BookshelfAdapter extends BaseAdapter {
  ...
  async execute(sql, params = []) {
    ...
    try {
      ...
      if (process.env.DEBUG_DB === 'true') {
        ...
        console.log(`[DB Bookshelf] ${duration}ms: ${sql.substring(0, 100)}${sql.length > 100 ? '...' : ''}`);
      }
      
      return Array.isArray(results[0]) ? [results[0], results[1]] : [results[0], []];
    } catch (error) {
      console.error('[DB Error Bookshelf]', error.message);
  ...
  async query(sql) {
    ...
    try {
      ...
      if (process.env.DEBUG_DB === 'true') {
        const duration = Date.now() - startTime;
        console.log(`[DB Query Bookshelf] ${duration}ms: ${sql.substring(0, 100)}${sql.length > 100 ? '...' : ''}`);
      }
      
      return Array.isArray(results[0]) ? [results[0], results[1]] : [results[0], []];
    } catch (error) {
      console.error('[DB Query Error Bookshelf]', error.message);
      console.error('[SQL]', sql.substring(0, 200));
      throw error;
    }
  }
