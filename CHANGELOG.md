...
- **API для записей звонков**: 
  - `GET /api/recording/:uniqueid` - скачать аудиозапись звонка по уникальному идентификатору
  - `GET /api/recording/:uniqueid/info` - получить метаданные записи звонка без скачивания
...
- ⚠️ sqlite3 содержит уязвимости в dev-зависимости node-tar (CVE-2021-32803, CVE-2021-32804)
  - Это транзитивная зависимость через `sqlite3 > node-gyp > tar`
  - Не влияет на production, так как sqlite3 используется только для локальных настроек
  - Исправление ожидается в будущих обновлениях sqlite3
